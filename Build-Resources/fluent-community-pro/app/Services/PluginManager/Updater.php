...
class Updater
{
    ...
    public function init()
    {
        ...
        add_action('fluent_plugins_renew_version_info_' . $this->slug, function ($isForced = false) {
            $optionValue = get_option($this->response_transient_key);
            if (!$isForced && $optionValue && !empty($optionValue['timeout'])) {
                $timeOut = $optionValue['timeout'];
                if (($timeOut - current_time('timestamp')) > 43200) { // for 12 hours
                    return false;
                }
            }

            $version_info = $this->api_request('plugin_latest_version', array('slug' => $this->slug));
            if (!$version_info || is_wp_error($version_info)) {
                return false;
            }
    ...
    public function show_changelog()
    {
        ...
        if (!current_user_can('update_plugins')) {
            wp_die(__('You do not have permission to install plugin updates', 'fluent-community-pro'), __('Error', 'fluent-community-pro'),
                array('response' => 403));
        }
    ...
    private function maybe_delete_transients()
    {
        ...
        if (isset($_GET['fluent-community-pro-check-update'])) {
            if (current_user_can('update_plugins')) {
                ...
                wp_redirect(admin_url('plugins.php?s=fluent-community&plugin_status=all'));
                exit();
            }
        }
    }
    ...
    {
        if (empty($expiration)) {
            $expiration = current_time('timestamp') + 86400;
        }
