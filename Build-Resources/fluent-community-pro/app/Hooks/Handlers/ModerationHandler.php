...
class ModerationHandler
{
    ...
    public function handlePostReportEmailAsync($reportId, $lastUserId)
    {
        ...
        $emailBody = $this->getFeedHtml($post, true);
    ...
    public function getCommentHtml($comment, $withPlaceholder = false)
    {
        ...
        $emailComposer->addBlock('button', __('Review the comment', 'fluent-community-pro'), [
            'link' => $postPermalink
        ]);
    ...
    public function getFeedHtml($feed, $withPlaceholder = false)
    {
        ...
        $emailComposer->addBlock('post_boxed_content', $feedHtml, [
            ...
            'title'      => $feed->title,
            'permalink'  => $postPermalink,
            'space_name' => $feed->space ? $feed->space->title : __('Community', 'fluent-community-pro'),
            'is_single'  => true
        ]);

        $emailComposer->addBlock('button', __('Review the post', 'fluent-community-pro'), [
            'link' => $postPermalink
        ]);
    ...
    private function getMediaHtml($meta, $postPermalink)
    {
        ...
        if ($mediaImage) {
            ...
            if ($mediaCount > 1) {
                $feedHtml .= '<p style="text-align: center; font-size: 14px; color: #666; margin-top: 10px;">' . \sprintf(_n('+%d more image', '+%d more images', $mediaCount - 1, 'fluent-community-pro'), $mediaCount - 1) . '</p>';
            }
            $feedHtml .= '</div>';
        }
