...
class ProAdminController extends Controller
{
    ...
    public function getUsers(Request $request)
    {
        if ($request->get('context') == 'add_manager') {
            $users = User::whereDoesntHave('community_role')
                ->searchBy($request->getSafe('search', 'sanitize_text_field'))
                ->paginate();
        } else {
            $users = User::searchBy($request->getSafe('search', 'sanitize_text_field'))
                ->paginate();
        }
    ...
    public function saveSidebarLink(Request $request)
    {
        ...
        if ($fromattedData['privacy'] == 'members_only') {
            ...
            $settings['membership_ids'] = $spaces->pluck('id')->toArray();
