	...
	function mycred_get_addon_defaults( $addon = '' ) {
		...
		switch ( $addon ) {
			...
			case 'woocommerce':
				$settings = array(
					'reward' => array(
						'types'   => array_map( fn( $type ) => 1, mycred_get_types() ),
...
if ( ! function_exists( 'mycred_encode_values' ) ) :
	function mycred_encode_values( $value ) {
		...
		$buycred_salt = mycred_get_option( 'mycred_buycred_salt' );

		if ( empty( $buycred_salt ) ) {
			
			$buycred_salt = wp_generate_password();
			mycred_update_option( 'mycred_buycred_salt', $buycred_salt );

		}
	
		$key    = sha1( $buycred_salt );
		$strLen = strlen( $value );
		$keyLen = strlen( $key );
		$j = 0;
		$crypttext = '';
...
if ( ! function_exists( 'mycred_create_toggle_field' ) ) :	
	function mycred_create_toggle_field( $atts = array(), $value = '', $checked = false, $echo = true ) {

		$wrapper_classes = 'mycred-toggle-wrapper';

		if ( isset( $atts['inline'] ) && $atts['inline'] == false ) {
			$wrapper_classes .= ' m-row';
		}
		...
		$input_attr = array( 'type' => 'checkbox', 'value' => $value );
		$label_attr = array();

		if ( ! empty( $atts['name'] ) ) {
			$input_attr['name'] = $atts['name'];
		}

		if ( ! empty( $atts['id'] ) ) {
			$input_attr['id'] = $atts['id'];
			$label_attr['for'] = $atts['id'];
		}
		...
		$field = '<div class="' . esc_attr( $wrapper_classes ) . '">';

		if ( isset( $atts['label'] ) && ! $atts['after'] ) {
			$field .= '<label '. mycred_get_attribute_html( $label_attr ) .'>' . esc_html( $atts['label'] ) . '</label>';
		}

		$field .= '<label class="mycred-toggle">';
		$field .= '<input '. mycred_get_attribute_html( $input_attr ) . mycred_checked( $checked, false ) . '>';
...
if ( ! function_exists( 'mycred_create_checkbox_field' ) ) :	
	function mycred_create_checkbox_field( $atts = array(), $value = '', $checked = false, $echo = true ) {

		$atts['type'] = 'checkbox';

		$field = mycred_build_checkbox_radio_fields( $atts, $value, $checked );
...
if ( ! function_exists( 'mycred_create_radio_field' ) ) :	
	function mycred_create_radio_field( $atts = array(), $value = '', $checked = false, $echo = true ) {
...
if ( ! function_exists( 'mycred_build_checkbox_radio_fields' ) ) :	
	function mycred_build_checkbox_radio_fields( $atts = array(), $value = '', $checked = false ) {

		if ( empty( $atts['type'] ) || ! in_array( $atts['type'], array( 'checkbox', 'radio' ) ) ) {
			return;
		}

		$wrapper_classes = 'mycred-checkbox-radio-wrapper ' . $atts['type'];
		...
		$input_attr = array( 'type' => $atts['type'], 'value' => $value );
		$label_attr = array();
		...
		$field .= '<label class="mycred-checkbox-radio">';
		$field .= '<input '. mycred_get_attribute_html( $input_attr ) . mycred_checked( $checked, false ) . '>';
		$field .= '</label>';
...
if ( ! function_exists( 'mycred_checked' ) ) :	
	function mycred_checked( $checked = false, $echo = true ) {
